...
def dwt_init(x):
    ...
    x_LH = -x1 + x2 - x3 + x4
    x_HH = x1 - x2 - x3 + x4
    # y = torch.zeros([4, 64, 32, 32])
    # torch.where(x_HL > (threshold * torch.max(x_HL)), x, y)
    # torch.clamp(input, min, max, out=None)
    # x_HL[x_HL < (threshold * torch.max(x_HL))] = 0
    # # x_HL = x_HL.cpu().detach().numpy()
    # x_LH = pywt.threshold(x_LH, threshold * max(x_LH))
    # x_HH = pywt.threshold(x_HH, threshold * max(x_HH))
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, nf=64):
        ...
        #     nn.Conv2d(nf, nf, 3, 1, 1, bias=True))
        #
        # self.conv_gelu1 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=True),
        #                                 nn.GELU())
        # self.conv_gelu2 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=True),
        #                                 nn.GELU())
        # self.conv_relu1 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=True),
        #                                 nn.ReLU(inplace=True))
        # self.avg_pool = nn.AdaptiveAvgPool2d(1)
        # self.conv_relu2 = nn.Sequential(nn.Conv2d(64, 4, kernel_size=1, stride=1, padding=0, bias=True),
        #                                 nn.ReLU(inplace=True))
        # self.conv_sigmoid = nn.Sequential(nn.Conv2d(4, 64, kernel_size=1, stride=1, padding=0, bias=True),
        #                                   nn.Sigmoid())
        #
        # self.conv_relu3 = nn.Sequential(nn.Conv2d(4, 64, kernel_size=1, stride=1, padding=0, bias=True),
        #                                 nn.ReLU(inplace=True))
        ...
        # self.conv_gelu2 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=True),
        #                                 nn.GELU())
        # self.conv_elu1 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=True),
        #                                 nn.ELU(inplace=True))
        # self.conv_elu2 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=True),
        #                                 nn.ELU(inplace=True))
        self.conv_relu1 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=True),
                                        nn.ReLU(inplace=True))
        self.conv_relu2 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=True),
                                        nn.ReLU(inplace=True))
        # self.avg_pool = nn.AdaptiveAvgPool2d(1)
        # self.conv_relu2 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=1, stride=1, padding=0, bias=True),
        #                                 nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        # x = torch.pow(torch.abs(x) + 1e-8, gamma=0.8)  # abs
        # x = self.conv_relu1(x)
        # x = self.conv_relu2(x)
        x = self.conv3(x)
        x = self.IDWT(x)
        ...
        x = x0 + x
        # a = np.array(x)
        # torch.save(x, '/home/yang/project/EGWVSR/codes/results/BIO/shiyanshjju/save_tensor5.pt')
        return x
...
class SRNet(nn.Module):
    ...
    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=16, upsample_func=None,
        ...
        # output conv.
        self.conv_out = nn.Conv2d(16, out_nc, 3, 1, 1, bias=True)
    ...
    def forward(self, lr_curr, hr_prev_tran):
        ...
        # dwt = DWT()
        # change_img_tensor = dwt(hr_prev_tran)
        # out = self.conv_in(torch.cat([lr_curr, change_img_tensor], dim=1))
...
class FRNet(BaseSequenceGenerator):
    ...
    def forward(self, lr_curr, lr_prev, hr_prev):
        ...
        # compute hr_curr
        hr_curr = self.srnet(lr_curr, hr_prev_warp)
    ...
    def forward_sequence(self, lr_data):
        ...
        hr_prev = self.srnet(
            lr_data[:, 0, ...],
            torch.zeros(n, c, hr_h, hr_w, dtype=torch.float32,
