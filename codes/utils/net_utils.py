...
def get_upsampling_func(scale=2, degradation='BI'):
    if degradation == 'BI':
        ...
        upsample_func = functools.partial(
            F.interpolate, scale_factor=scale, mode='bicubic',
            align_corners=False)
...
class BicubicUpsample(nn.Module):
    ...
    def forward(self, input):
        ...
        input = F.pad(input, (1, 2, 1, 2), mode='replicate')#当pad有四个参数，代表对最后两个维度扩充，pad = (左边填充数， 右边填充数， 上边填充数， 下边填充数)
